...
.btn {
    ...
    body{
        width: 100%;
        height:100%;
        background:url("../../resources/admin/login/images/aa1.jpg") no-repeat;
        background-size:100% ;
    }
...
<body>
        <div class="box"></div>
        <!--头部-->
        <%--<div class="c_top"><img src="../../resources/admin/login/images/aa.jpg" style="width: 1200px;height:260px;" alt=""> </div>--%>
        <!--主体-->
        <div id="contain">
            <!--tab选项卡-->
          <ul class="tabs">
                ...
                <tbody>
                   ...
                   <tr>
                    ...
                    <form id="book_form" method="post">
                           <input type="hidden" id ="de_book" value="${bookOrder.id}">
                           <input type="button" value="取消" class="want-buy" id="delete_order" onclick="window.location.href='${pageContext.request.contextPath}/home/account/delete?id=${bookOrder.id}'"></form>
                       </td>
                   </tr>
                   </c:forEach>
                </tbody>
 ...
 <script>
    ...
    var $div=$("div.content>div."+href);
     ...
     <%--$(".want-buy").click(function(){--%>
         <%--var id=$("#de_book").val()--%>
         <%--alert("messge"+id);--%>
         <%--var statu = confirm("确定要取消订单吗？当天入住时间6点之前免费取消，之后取消扣除10%房费！");--%>
         <%--if(!statu){--%>
             <%--return false;--%>
         <%--}--%>

         <%--$.ajax({--%>
             <%--url:'${pageContext.request.contextPath}/home/account/delete',--%>
             <%--dataType:'json',--%>
             <%--type:'post',--%>
             <%--data:{id:id},--%>
             <%--success:function(data){--%>
                 <%--if(data.type == 'success'){--%>
                     <%--alert('信息提示','取消成功！','info');--%>
                 <%--}else{--%>
                     <%--alert('信息提示',data.msg,'warning');--%>
                 <%--}--%>
             <%--}--%>
         <%--});--%>
     <%--});--%>
    //修改个人信息
     $("#update-info-btn").click(function(){
    	$.ajax({
    		url:'${pageContext.request.contextPath}/home/account/update_info',
    		type:'post',
    		dataType:'json',
    ...
    //修改密码
     $("#update-password-btn").click(function(){
    	...
    	$.ajax({
    		url:'${pageContext.request.contextPath}/home/account/update_pwd',
    		type:'post',
    		dataType:'json',
