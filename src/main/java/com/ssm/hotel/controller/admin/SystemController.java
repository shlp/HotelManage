...
import com.ssm.hotel.util.CpachaUtil;
import com.ssm.hotel.util.MenuUtil;
...
public class SystemController {
	...
	@ResponseBody
	public Map<String, String> loginAct(Account user, String cpacha, HttpServletRequest request){
		...
		if(!cpacha.toUpperCase().equals(loginCpacha.toString().toUpperCase())){
			ret.put("type", "error");
			ret.put("msg", "验证码错误！");
			logService.add("用户名为"+user.getName()+"的用户登录时输入验证码错误!");
			return ret;
		}
		Account findByUsername = accountService.findByName(user.getName());
		if(findByUsername == null){
			ret.put("type", "error");
			ret.put("msg", "该用户名不存在！");
			logService.add("登录时，用户名为"+user.getName()+"的用户不存在!");
			return ret;
		}
		if(findByUsername.getRoleId()==2){
			ret.put("type", "error");
			ret.put("msg", "没有权限！");
			logService.add("用户名为"+user.getName()+"的用户登录时无权限!");
			return ret;
		}
		if(!user.getPassword().equals(findByUsername.getPassword())){
			ret.put("type", "error");
			ret.put("msg", "密码错误！");
			logService.add("用户名为"+user.getName()+"的用户登录时输入密码错误!");
			return ret;
		}
		//说明用户名密码及验证码都正确
		//此时需要查询用户的角色权限
		...
		ret.put("msg", "登录成功！");
		logService.add("用户名为{"+user.getName()+"}，角色为{"+role.getName()+"}的用户登录成功!");
		return ret;
	}
	...
	public Map<String, String> editPasswordAct(String newpassword,String oldpassword,HttpServletRequest request){
		...
		Account user = (Account) request.getSession().getAttribute("admin");
		...
		if(accountService.editPassword(user) <= 0){
			ret.put("type", "error");
		...
		ret.put("type", "success");
		ret.put("msg", "密码修改成功！");
		logService.add("用户名为{"+user.getName()+"}，的用户成功修改密码!");
		return ret;
	}
