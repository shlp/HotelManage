    ...
    <div id="wu-toolbar">
        ...
        <div class="wu-toolbar-search">
            ...
            <select id="search-status" class="easyui-combobox" panelHeight="auto" style="width:120px">
                ...
                <option value="3">检查无误</option>
                <option value="4">检查有误</option>
                <option value="5">维修中</option>
                <option value="6">加急打扫</option>
            </select>
	...
	<form id="add-form" method="post">
        <table>
            <tr>
                ...
                <td valign="middle">
                	<img id="preview-photo" style="float:left;" src="/HotelManageSSM/resources/admin/easyui/images/user_photo.jpg" width="100px">
            ...
            <tr>
                ...
                <td><input type="text" id="add-photo" name="photo" value="/HotelManageSSM/resources/admin/easyui/images/user_photo.jpg" readonly="readonly" class="wu-text " /></td>
            </tr>
            <tr>
	...
	<form id="edit-form" method="post">
        ...
        <table>
           <tr>
                <td width="60" align="right">图片预览:</td>
                <td valign="middle">
                	<img id="edit-preview-photo" style="float:left;" src="/HotelManageSSM/resources/admin/easyui/images/user_photo.jpg" width="100px">
                	<a style="float:left;margin-top:40px;" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-upload" onclick="uploadPhoto()" plain="true">上传图片</a>
                </td>
            </tr>
            <tr>
                <td width="60" align="right">房间图片:</td>
                <td><input type="text" id="edit-photo" name="photo" value="/HotelManageSSM/resources/admin/easyui/images/user_photo.jpg" readonly="readonly" class="wu-text " /></td>
            </tr>
           <tr>
                <td align="right">房间编号:</td>
...
<!-- 检查是否有误窗口 -->
<div id="editRemark-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'" style="width:450px; padding:10px;">
    <form id="editRemark-form" method="post">
        <input type="hidden" name="id" id="editRemark-id">
        <table>
            <tr>
                <td align="right">状态:</td>
                <td>
                    <select id="editRemark-status" name="status" class="easyui-combobox" panelHeight="auto" style="width:268px">
                        <option value="-1">全部</option>
                        <option value="3">检查无误</option>
                        <option value="4">检查有误</option>
                    </select>
            ...
            <tr>
                ...
                <td><textarea id="editRemark-remark" name="remark" rows="6" class="wu-textarea" style="width:260px"></textarea></td>
            </tr>
        </table>
    </form>
</div>
 ...
 */
function checkRoom(){
    var validate = $("#editRemark-form").form("validate");
    if(!validate){
        $.messager.alert("消息提醒","请检查你输入的数据!","warning");
        return;
    }
    var data = $("#editRemark-form").serialize();
    $.ajax({
        url:'checkRoom',
        dataType:'json',
        type:'post',
        ...
        success:function(data){
            if(data.type == 'success'){
                $.messager.alert('信息提示','修改成功！','info');
                $('#editRemark-dialog').dialog('close');
                $('#data-datagrid').datagrid('reload');
            }else{
                $.messager.alert('信息提示',data.msg,'warning');
            }
...
function updateRoom(){
    $.messager.confirm('信息提示','确定要维修吗？', function(result){
        if(result){
...
function swapRoom(){
    $.messager.confirm('信息提示','确定要打扫该房间吗？', function(result){
        if(result){
            var item = $('#data-datagrid').datagrid('getSelected');
            if(item == null || item.length == 0){
                $.messager.alert('信息提示','请选择要打扫的房间！','info');
                return;
            }
...
function swapok(){
    $.messager.confirm('信息提示','确定要确认已打扫房间吗？', function(result){
        if(result){
            var item = $('#data-datagrid').datagrid('getSelected');
            if(item == null || item.length == 0){
...
function openCheckRoom(){
    ...
    $('#editRemark-dialog').dialog({
        ...
        onBeforeOpen:function(){
            //$("#add-form input").val('')
            $("#editRemark-id").val(item.id);
            $("#editRemark-status").combobox('setValue',item.status);
            $("#editRemark-remark").val(item.remark);
        }
    });
}
	/**
